...
class Robot:
    ...
    def data_analysis(self):
        x = [-self.scan_data[i] * math.cos(self.x_data[i] * math.pi / 180) for i in range(225, 315)]
        y = [self.scan_data[i] * math.sin(self.x_data[i] * math.pi / 180) for i in range(225, 315)]
        X = []
        Y = []
        x_panneau = []
        y_panneau = []
        for i in range(len(x)):
            if math.sqrt(x[i]**2 + y[i]**2) < 700 and y[i] != 0:
                X.append(x[i])
                Y.append(y[i])
        if len(X) != 0:
